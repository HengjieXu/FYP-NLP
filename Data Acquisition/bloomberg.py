...
class Bloomberg:
    ...
    def getNews(self):
        ...
        for item in items_all:
            value = True
            if item['headline'].find('Bulletin') >= 0:
                continue
            for entity in list:
                if entity['title'] == item['headline']:
                    value = False
                    break
            if value:
                if item['storyType'] == 'Article' and item.get('headline').lower().find(headline_name.lower()) >= 0:
                    ...
                    if html is not None:
                        ...
                        news = re.sub(r'\([^)]*\)', '', news)
                        rmv = 'This column does not necessarily reflect'
                        if rmv in news:
                            idx = news.index(rmv)
                            news = news[:idx]

                        futile = 'Follow @Brexit for all the latest news, and sign up to our daily Brexit Bulletin newsletter.'

                        if futile in news:
                            news = news.replace(futile, "")
