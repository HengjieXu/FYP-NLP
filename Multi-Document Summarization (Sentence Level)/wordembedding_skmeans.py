...
for c in range(10):
    ...
    print idx
    for l in idx:
        simi = 1 - spatial.distance.cosine(centroid, results[l, :])
        simi_list.append(simi)
    simi_list.sort(reverse=True)
    for l in idx:
        if len(sent_list_old[l].split()) <= 6:
            continue
        cos_simi = 1 - spatial.distance.cosine(centroid, results[l, :])
...
for id in summa_index:
    print 'Cluster ' + str(seq+1) + ': ' + sent_list_old[id] + '\n\n'
    seq += 1